...
# PLOTTING FUNCTION
# -----------------------------
def make_2x2_case_figure(metric_indices, x_tick_numbers, out_stem):
    """
    ...
    fig, axes = plt.subplots(2, 2, figsize=(6.69, 4.95))
    axes = axes.ravel()

    x = np.array(x_tick_numbers, dtype=float)

    for ax, (case_letter, sheet) in zip(axes, CASES):
        cdf = success_only(central_wb[sheet])
        sdf = success_only(symbolic_wb[sheet])

        c_means = mean_vector(cdf, metric_names)
        s_means = mean_vector(sdf, metric_names)
        ...
        # Right-side A/B/C/D label
        ax_r = ax.twinx()
        ax_r.set_ylabel(case_letter, rotation=0, labelpad=10, va="center")
        ax_r.set_yticks([])
...
make_2x2_case_figure(
    ...
    out_stem="cases_AtoD_metrics_7to12"
)
