...
def make_ipopt(output_file):
    ipopt_exe = os.environ.get("IPOPT_BIN_OVERRIDE") or IPOPT_BIN
    linear_solver = os.environ.get("IPOPT_LINEAR_SOLVER", IPOPT_LINEAR_SOLVER_DEFAULT)
    set = pyo.SolverFactory("ipopt", executable=ipopt_exe)
    set.options["linear_solver"] = linear_solver
    # # ---- IPOPT logging ----
...
class PDE_diffusion(Experiment):
    ...
    def finalize_model(self):
        ...
        TransformationFactory("dae.finite_difference").apply_to(m, nfe=self.nfe_x, wrt=m.x)
        TransformationFactory("dae.finite_difference").apply_to(m, nfe=self.nfe_t, wrt=m.t)
...
def parse_ipopt_out(ipopt_out_path):
    ...
    stats["obj_eval_count"] = grab_int(r"Number of objective function evaluations\s*=\s*(\d+)")
    stats["grad_eval_count"] = grab_int(r"Number of objective gradient evaluations\s*=\s*(\d+)")
    stats["eq_con_eval_count"] = grab_int(r"Number of equality constraint evaluations\s*=\s*(\d+)")
    stats["ineq_con_eval_count"] = grab_int(r"Number of inequality constraint evaluations\s*=\s*(\d+)")
    stats["eq_con_jac_eval_count"] = grab_int(r"Number of equality constraint Jacobian evaluations\s*=\s*(\d+)")
    stats["ineq_con_jac_eval_count"] = grab_int(r"Number of inequality constraint Jacobian evaluations\s*=\s*(\d+)")
    stats["hess_eval_count"] = grab_int(r"Number of Lagrangian Hessian evaluations\s*=\s*(\d+)")

    stats["ipopt_cpu_secs_no_eval"] = grab_float(r"Total CPU secs in IPOPT \(w/o function evaluations\)\s*=\s*([0-9.]+)")
    stats["nlp_cpu_secs_eval"] = grab_float(r"Total CPU secs in NLP function evaluations\s*=\s*([0-9.]+)")
...
def main():
    ...
    doe_obj = DesignOfExperiments(
        ...
        solver=make_ipopt(ipopt_out_path),#SolverFactory('IPOPT'), #, options={'linear_solver': 'mumps'}
        tee=False,
    ...
    print("=== INITIAL DESIGN (central) [pre-create] ===")
    ...
    try:
        ...
        print("=== INITIAL DESIGN (central) [exception] ===")
        _print_design("design_values:")
        raise
    ...
    output_path = Path(args.out_json) if args.out_json else run_dir / f"pde_diffusion_central_nfe_t{args.nfe_t}_nfe_x{args.nfe_x}.json"
