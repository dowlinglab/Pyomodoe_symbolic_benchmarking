...
CASES = [
    ("case1_reactor",   "4_state_reactor_central.py", []),
    ("case2_2paramsin", "two_param_sin_central.py",   ["--wdir"]),
    ("case3_scalarPDE", "PDE_diffusion_central.py",   ["--wdir"]),
    ("case4_6param",    "4st_6pmt_central.py",        ["--wdir"]),
]

N_RUNS = 10
OUT_XLSX = Path("doe_benchmark_central.xlsx")
...
def parse_stdout_metrics(blob: str) -> dict:
    ...
    d["ipopt_cpu_no_eval_s"] = grab_float(
        blob, r"Total CPU secs in IPOPT \(w/o function evaluations\)\s*=\s*([0-9.eE+-]+)"
    )
    d["ipopt_cpu_nlp_eval_s"] = grab_float(
        blob, r"Total CPU secs in NLP function evaluations\s*=\s*([0-9.eE+-]+)"
    )
    d["ipopt_exit"] = grab_str(blob, r"EXIT:\s*([^\r\n]+)")

    # IPOPT diagnostics (scaled/unscaled)
